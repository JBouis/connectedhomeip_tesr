...
#define USER_LED_Pin USER_LED1_Pin
#define USER_LED_GPIO_Port USER_LED1_GPIO_Port
...
CHIP_ERROR AppTask::StartAppTask()
{
    ...
    {
        STM32_LOG("Failed to allocate app event queue");
...
CHIP_ERROR AppTask::Init()
{
    ...
    AppLED.Init(USER_LED_GPIO_Port, USER_LED_Pin);
    // AppButton.Init();
...
void AppTask::AppTaskMain(void * pvParameter)
{
    ...
    {
        STM32_LOG("AppTask.Init() failed due to %" CHIP_ERROR_FORMAT, err.Format());
        return;
    }

    STM32_LOG("App Task started");
...
void AppTask::PostEvent(const AppEvent * aEvent)
{
    ...
    //     }
    //     if (!status)
    //         STM32_LOG("Failed to post event to app task event queue");
    // }
    // else
    // {
    //     STM32_LOG("Event Queue is NULL should never happen");
    // }

    STM32_LOG("AppTask::PostEvent");
}
...
void AppTask::DispatchEvent(AppEvent * aEvent)
{
    ...
    {
        STM32_LOG("Event received with no handler. Dropping event.");
    }
}
